...
def main(input, output, image_type, gpu, model_path, model_name, preview, octaves, octave_scale, iterations, jitter, zoom, stepsize, blend, layers, guide_image, start_frame, end_frame, verbose):
    ...
    if stepsize is None:
        stepsize = 1.5
    if blend is None:
        blend = 0.5  # can be nr (constant), random, or loop
    if verbose is None:
        verbose = 1
    if layers is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-it', '--image_type', help='Specify whether jpg or png ', required=True)
    parser.add_argument('--gpu', default=None, help='Switch for gpu computation.')  # int can chose index of gpu, if there are multiple gpu's to chose from
    parser.add_argument('-t', '--model_path', dest='model_path', default='../caffe/models/bvlc_googlenet/', help='Model directory to use')
    parser.add_argument('-m', '--model_name', dest='model_name', default='bvlc_googlenet.caffemodel', help='Caffe Model name to use')
    parser.add_argument('-p', '--preview', type=int, required=False, help='Preview image width. Default: 0')
    parser.add_argument('-oct', '--octaves', type=int, required=False, help='Octaves. Default: 4')
    parser.add_argument('-octs', '--octavescale', type=float, required=False, help='Octave Scale. Default: 1.4',)
    parser.add_argument('-itr', '--iterations', type=int, required=False, help='Iterations. Default: 10')
    parser.add_argument('-j', '--jitter', type=int, required=False, help='Jitter. Default: 32')
    parser.add_argument('-z', '--zoom', type=int, required=False, help='Zoom in Amount. Default: 1')
    parser.add_argument('-s', '--stepsize', type=float, required=False, help='Step Size. Default: 1.5')
    parser.add_argument('-b', '--blend', type=str, required=False, help='Blend Amount. Default: "0.5" (constant), or "loop" (0.5-1.0), or "random"')
    parser.add_argument('-l', '--layers', nargs='+', type=str, required=False, help='Array of Layers to loop through. Default: [customloop] - or choose ie [inception_4c/output] for that single layer')
    parser.add_argument('-v', '--verbose', type=int, required=False, help='verbosity [0-3]')
    parser.add_argument('-gi', '--guide_image', required=False, help='path to guide image')
    parser.add_argument('-sf', '--start_frame', type=int, required=False, help='starting frame nr')
    parser.add_argument('-ef', '--end_frame', type=int, required=False, help='end frame nr')
    parser.add_argument('-e', '--extract', type=int, required=False, help='Extract frames from video')
